...
def build_and_run():
    ...
    print('*************Right Before PiCamera initiali**************')
    command = f'free -h'
    subprocess.call(command, shell=True)
    ...
    camera.start_and_capture_file(image_file)
    end_cam_time = time.time()
    print('camera capture time: ', str(end_cam_time - start_cam_time))
    
    print('*************Right After PiCamera initialized**************')
    command = f'free -h'
    subprocess.call(command, shell=True)
